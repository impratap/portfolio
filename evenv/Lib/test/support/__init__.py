...
def check_sanitizer(*, address=False, memory=False, ub=False):
    ...
    address_sanitizer = (
        "-fsanitize=address" in _cflags or "--with-address-sanitizer" in _config_args
    )
    ub_sanitizer = (
...
class TestStats:
    ...
    def from_doctest(results):
        return TestStats(results.attempted, results.failed)
...
def run_doctest(module, verbosity=None, optionflags=0):
    ...
    results = doctest.testmod(module, verbose=verbosity, optionflags=optionflags)
    if results.failed:
        stats = TestStats.from_doctest(results)
        raise TestFailed(
            f"{results.failed} of {results.attempted} " f"doctests failed", stats=stats
        )
    if verbose:
        print(
